...
func (ctx *CallContext) Execute(txEnv *txs.Envelope, txRec *txs.Receipt) error {
	...
	if tx.CreateContract() {
		...
		log.Debug("Creating new contract", "init_code", tx.Data())
	} else {
	...
	if ctx.Committing {
		ret := ctx.Deliver(txEnv, caller, callee)
...
func (ctx *CallContext) Deliver(env *txs.Envelope, caller, callee *account.Account) (ret sputnikvm.Output) {
	...
	tx, _ := env.Tx.(*tx.CallTx)
	adapter := sputnikvm.GallacticAdapter{ctx.BC, ctx.Cache, caller,
		callee, tx.GasLimit(), tx.Amount(), tx.Data(), caller.Sequence(), env.Hash()}
